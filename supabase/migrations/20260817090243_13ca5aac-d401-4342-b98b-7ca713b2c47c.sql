...
-- MIGRATION COMPLETAMENTE ADDITIVA
-- =====================================================================
...
AS $$
BEGIN
  -- Bypass consentito SOLO a una sessione tecnica diretta (owner del database),
  -- mai a service_role/authenticated/anon e mai via PostgREST o Edge Function.
...
REVOKE UPDATE, DELETE, TRUNCATE ON public.product_source_snapshots FROM service_role;
...
REVOKE UPDATE, DELETE, TRUNCATE ON public.product_field_history FROM service_role;
...
-- ---------------------------------------------------------------------
-- 8. HARDENING FASE B — grant minimi sulle funzioni
-- ---------------------------------------------------------------------
REVOKE ALL ON FUNCTION public.can_publish_products(uuid) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.can_edit_products(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.can_publish_products(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.can_edit_products(uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.deny_snapshot_mutation() FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.assert_ai_field_allowed() FROM PUBLIC, anon, authenticated, service_role;